...
* @returns {StringArray} list of filtered files
*/
...
function readJSONFiles( files ) {
	...
	for ( i = 0; i < files.length; i++ ) {
		f = files[ i ];
		fpath = resolve( DATA_DIR, f );
		ext = extname( f );
		out[ replace( f, ext, '' ) ] = readJSON( fpath, OPTS );
	}
	return out;
}
